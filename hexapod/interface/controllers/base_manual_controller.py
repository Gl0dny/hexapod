...
class ManualHexapodController(threading.Thread, ABC):
    ...
    def __init__(
        ...
        try:
            self.gait_stance_height = float(
                self.task_interface.hexapod.gait_params.get("translation", {}).get(
                    "stance_height", 0.0
                )
            )
        except Exception:
    ...
    def reset_position(self) -> None:
        ...
        try:
            ...
            if self.current_mode == self.GAIT_CONTROL_MODE:
                ...
                self.task_interface.hexapod.gait_generator.create_gait(
                    "tripod", **translation_params
                )
                self.translation_gait = (
                    self.task_interface.hexapod.gait_generator.current_gait
                )
                self.task_interface.hexapod.gait_generator.create_gait(
                    "tripod", **rotation_params
                )
                self.rotation_gait = (
                    self.task_interface.hexapod.gait_generator.current_gait
                )
    ...
    def start_gait_control(
        ...
        if translation_params is None:
            translation_params = self.task_interface.hexapod.gait_params.get(
                "translation", {}
            ).copy()
        if rotation_params is None:
            rotation_params = self.task_interface.hexapod.gait_params.get(
                "rotation", {}
            ).copy()
    ...
    def _process_stance_height_delta(self, inputs: Dict[str, Any]) -> None:
        ...
        if (
            abs(stance_height_delta) > 0.0
            and self.current_mode == self.GAIT_CONTROL_MODE
        ):
            old_stance = self.gait_stance_height
            self.gait_stance_height = max(
                self.GAIT_MIN_STANCE_HEIGHT,
                min(
                    self.GAIT_MAX_STANCE_HEIGHT,
                    self.gait_stance_height + stance_height_delta,
                ),
            )
            logger.gamepad_mode_info(
                f"Gait stance height: {self.gait_stance_height:.1f} mm"
            )
    ...
    def _process_body_control(self, inputs: Dict[str, Any]) -> None:
        ...
        tx = (
            inputs.get("tx", 0.0) * self.translation_sensitivity * self.TRANSLATION_STEP
        )
        ty = (
            inputs.get("ty", 0.0) * self.translation_sensitivity * self.TRANSLATION_STEP
        )
        ...
        if any(abs(val) > 0.01 for val in [tx, ty, tz, roll, pitch, yaw]):
            try:
                ...
                self.task_interface.hexapod.move_body(
                    tx=tx, ty=ty, tz=tz, roll=roll, pitch=pitch, yaw=yaw
                )
    ...
    def _process_gait_control(self, inputs: Dict[str, Any]) -> None:
        ...
        if (
            not self.marching_enabled
            and abs(direction_x) < 0.01
            and abs(direction_y) < 0.01
    ...
    def _process_sensitivity_deltas(self, inputs: Dict[str, Any]) -> None:
        ...
        if sensitivity_deltas.get("translation_delta", 0.0) != 0.0:
            if self.current_mode == self.BODY_CONTROL_MODE:
                self.translation_sensitivity = max(
                    self.MIN_SENSITIVITY,
                    min(
                        self.MAX_SENSITIVITY,
                        self.translation_sensitivity
                        + sensitivity_deltas["translation_delta"],
                    ),
                )
                logger.gamepad_mode_info(
                ...
                self.gait_direction_sensitivity = max(
                    self.MIN_SENSITIVITY,
                    min(
                        self.MAX_SENSITIVITY,
                        self.gait_direction_sensitivity
                        + sensitivity_deltas["translation_delta"],
                    ),
                )
                logger.gamepad_mode_info(
        ...
        if sensitivity_deltas.get("rotation_delta", 0.0) != 0.0:
            if self.current_mode == self.BODY_CONTROL_MODE:
                self.rotation_sensitivity = max(
                    self.MIN_SENSITIVITY,
                    min(
                        self.MAX_SENSITIVITY,
                        self.rotation_sensitivity
                        + sensitivity_deltas["rotation_delta"],
                    ),
                )
                logger.gamepad_mode_info(
                ...
                self.gait_rotation_sensitivity = max(
                    self.MIN_SENSITIVITY,
                    min(
                        self.MAX_SENSITIVITY,
                        self.gait_rotation_sensitivity
                        + sensitivity_deltas["rotation_delta"],
                    ),
                )
                logger.gamepad_mode_info(
    ...
    def show_gait_status(self) -> None:
        ...
        def _get_direction_name(direction_tuple: tuple[float, float]) -> str:
            ...
            for name, vec in BaseGait.DIRECTION_MAP.items():
                ...
                dist = (
                    (direction_tuple[0] - vec[0]) ** 2
                    + (direction_tuple[1] - vec[1]) ** 2
                ) ** 0.5
                if dist < min_dist:
                    min_dist = dist
                    closest_name = name
        ...
        if self.current_gait == self.translation_gait:
            ...
            lines.append(
                f"  Step Radius: {getattr(self.translation_gait, 'step_radius', 'N/A')} mm"
            )
            lines.append(
                f"  Leg Lift Distance: {getattr(self.translation_gait, 'leg_lift_distance', 'N/A')} mm"
            )
            lines.append(
                f"  Dwell Time: {getattr(self.translation_gait, 'dwell_time', 'N/A')}s"
            )
        if self.current_gait == self.rotation_gait:
            lines.append(f"- Rotation Gait Parameters:")
            lines.append(
                f"  Step Radius: {getattr(self.rotation_gait, 'step_radius', 'N/A')} mm"
            )
            lines.append(
                f"  Leg Lift Distance: {getattr(self.rotation_gait, 'leg_lift_distance', 'N/A')} mm"
            )
            lines.append(
                f"  Dwell Time: {getattr(self.rotation_gait, 'dwell_time', 'N/A')}s"
            )
        # Try to get current phase/state information from gait generator
        current_state = getattr(
            self.task_interface.hexapod.gait_generator, "current_state", None
        )
        if current_state:
            ...
            lines.append(
                f"- Stance Legs: {getattr(current_state, 'stance_legs', 'N/A')}"
            )
            lines.append(
                f"- Dwell Time: {getattr(current_state, 'dwell_time', 'N/A')}s"
            )
    ...
    def _on_mode_toggled(self, new_mode: str) -> None:
        ...
        if new_mode == self.BODY_CONTROL_MODE:
            self.task_interface.lights_handler.pulse_smoothly(
                base_color=ColorRGB.BLUE, pulse_color=ColorRGB.BLACK, pulse_speed=0.05
            )
    ...
    def toggle_voice_control_mode(self) -> None:
        ...
        if self.voice_control is None:
            logger.warning(
                "Voice control is not available. Cannot switch to voice control mode."
            )
            return
