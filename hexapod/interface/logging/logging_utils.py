    ...
    if config_file.is_file():
        ...
        for handler_name, handler in config.get("handlers", {}).items():
            ...
            if filename:
                ...
                )  # Set to log_dir/<basename>

        # Override log levels if specified
        config = override_log_levels(config, log_level)

        logging.config.dictConfig(config)
        queue_handler = logging.getLogger("root").handlers[
            0
        ]  # Assuming queue_handler is the first handler
        ...
        raise FileNotFoundError(
            f"Logging configuration file not found at {config_file}. Custom logging configuration is required."
        )
