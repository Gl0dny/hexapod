...
class Calibration:
    def __init__(self, hexapod: Hexapod, calibration_data_path: Path) -> None:
        ...
        self.calibration_positions = {
            "calibration_init": [
                (
                    0.0,
                    self.hexapod.femur_params["angle_max"],
                    self.hexapod.tibia_params["angle_max"],
                ),
                (
                    0.0,
                    self.hexapod.femur_params["angle_max"],
        ...
        try:
            for i, leg in enumerate(self.hexapod.legs):
                if stop_event and stop_event.is_set():
                    logger.error(
                        "Calibration interrupted before starting Leg {}.".format(i)
                    )
                    return
                ...
                for joint_name in ["coxa", "femur", "tibia"]:
                    if stop_event and stop_event.is_set():
                        logger.error(
                            "Calibration interrupted during calibration of Leg {}, Joint {}.".format(
                                i, joint_name
                            )
                        )
                        return
                    ...
                    while not calibration_success:
                        ...
                        if joint.invert:
                            self._calibrate_servo_min_inverted(
                                i, joint_name, stop_event
                            )
                            self._calibrate_servo_max_inverted(
                                i, joint_name, stop_event
                            )
                        else:
                        ...
                        if stop_event and stop_event.is_set():
                            logger.error(
                                "Calibration interrupted after calibrating Leg {}, Joint {}.".format(
                                    i, joint_name
                                )
                            )
                            return

                        calibration_success = self._check_zero_angle(
                            i, joint_name, stop_event
                        )
                    ...
                    self.hexapod.move_leg_angles(
                        leg_index=i,
                        coxa_angle=self.calibration_positions["calibration_init"][i][0],
                        femur_angle=self.calibration_positions["calibration_init"][i][
                            1
                        ],
                        tibia_angle=self.calibration_positions["calibration_init"][i][
                            2
                        ],
                    )
                    logger.debug(f"Set Leg {i} to calibration position.")
    ...
    def load_calibration(self) -> None:
        ...
        try:
                ...
                for joint_name in ["coxa", "femur", "tibia"]:
                    ...
                    if servo_min and servo_max:
                        if (
                            ...
                            and Joint.SERVO_INPUT_MIN * Joint.SERVO_UNIT_MULTIPLIER
                            <= servo_max
                            <= Joint.SERVO_INPUT_MAX * Joint.SERVO_UNIT_MULTIPLIER
                            and servo_min < servo_max
                        ):
                            self._calibrate_servo(i, joint_name, servo_min, servo_max)
                            logger.debug(
                                f"Loaded calibration for leg {i} {joint_name}: servo_min={servo_min}, servo_max={servo_max}"
                            )
                        else:
                            logger.warning(
                                f"Calibration values for leg {i} {joint_name} are invalid (servo_min: {servo_min}, servo_max: {servo_max}). Using default values: servo_min={Joint.SERVO_INPUT_MIN * Joint.SERVO_UNIT_MULTIPLIER}, servo_max={Joint.SERVO_INPUT_MAX * Joint.SERVO_UNIT_MULTIPLIER}."
                            )
                            self._calibrate_servo(
                                i,
                                joint_name,
                                Joint.SERVO_INPUT_MIN * Joint.SERVO_UNIT_MULTIPLIER,
                                Joint.SERVO_INPUT_MAX * Joint.SERVO_UNIT_MULTIPLIER,
            ...
            logger.exception(
                f"{self.calibration_data_path} not found. Using default calibration values. Run calibrate_all_servos() to set new values."
            )
        except json.JSONDecodeError as e:
            logger.exception(
                f"Error decoding {self.calibration_data_path}: {e}. Using default calibration values. Run calibrate_all_servos() to set new values."
            )
    ...
    def _calibrate_servo_min_inverted(
        self,
        leg_index: int,
        joint_name: str,
        stop_event: Optional[threading.Event] = None,
    ...
    def _calibrate_servo_max_inverted(
        self,
        leg_index: int,
        joint_name: str,
        stop_event: Optional[threading.Event] = None,
    ...
    def _check_zero_angle(
        ...
        while not calibrated_zero:
            if stop_event and stop_event.is_set():
                logger.error(
                    f"Calibration interrupted during zero angle check of Leg {leg_index} {joint_name}."
                )
                return False
            try:
                joint = getattr(self.hexapod.legs[leg_index], joint_name)
        ...
        while servo_input is None:
            ...
            if servo_input_str is not None:
                try:
                    ...
                    print(
                        f"Invalid input. Please enter an integer value for {servo_label}."
                    )
                    print(prompt, end="", flush=True)
                    servo_input = None
            else:
    ...
    def _confirm_calibration_done(
        self, servo_label: str, stop_event: Optional[threading.Event] = None
    ) -> bool:
        """
    ...
    def _validate_servo_input(
        self, servo_label: str, servo_input: int, invert: bool = False
    ) -> bool:
        """
        ...
        if not (Joint.SERVO_INPUT_MIN <= servo_input <= Joint.SERVO_INPUT_MAX):
            print(
                f"Error: {servo_label} must be between {Joint.SERVO_INPUT_MIN} and {Joint.SERVO_INPUT_MAX}."
            )
            return False
        ...
        if invert:
            if servo_label == "servo_min" and servo_input >= Joint.SERVO_INPUT_MAX:
                print(
                    f"Error: {servo_label} must be less than current maximum servo_max input ({Joint.SERVO_INPUT_MAX})."
                )
                return False
            elif servo_label == "servo_max" and servo_input <= Joint.SERVO_INPUT_MIN:
                print(
                    f"Error: {servo_label} must be greater than current minimum servo_min input ({Joint.SERVO_INPUT_MIN})."
                )
                return False
        else:
            if servo_label == "servo_min" and servo_input >= Joint.SERVO_INPUT_MAX:
    ...
    def _calibrate_joint(
        ...
        """
        logger.debug(
            f"Entering {servo_label} calibration for Leg {leg_index} {joint_name}."
        )
        calibrated = False
        while not calibrated:
            if stop_event and stop_event.is_set():
                logger.error(
                    f"Calibration interrupted during {servo_label} of Leg {leg_index} {joint_name}."
                )
                return
            try:
                joint = getattr(self.hexapod.legs[leg_index], joint_name)
                ...
                print(
                    f"Expected {angle_label} ({expected_angle}°) corresponds to {servo_label} (Expected Value: {expected_servo})."
                )

                servo_input = self._prompt_servo_value(
                    leg_index, joint_name, servo_label, stop_event
                )
                ...
                if servo_label == "servo_min":
                    self._calibrate_servo(
                        leg_index, joint_name, servo_value, joint.servo_max
                    )
                else:
                    self._calibrate_servo(
                        leg_index, joint_name, joint.servo_min, servo_value
                    )
                ...
                logger.debug(
                    f"Set {joint_name} of Leg {leg_index} to {angle_label}: {expected_angle}°"
                )
    ...
    def _calibrate_servo(
        self, leg_index: int, joint: str, servo_min: int, servo_max: int
    ) -> None:
        """
        ...
        try:
            if 0 <= leg_index < len(self.hexapod.legs):
                ...
                if joint == "coxa":
                    ...
                    logger.error(
                        "Invalid joint name. Choose 'coxa', 'femur', or 'tibia'."
                    )
            else:
                logger.error("Invalid leg index. Must be between 0 and 5.")
        except Exception as e:
            logger.exception(
                f"Error updating servo calibration for leg {leg_index} {joint}: {e}"
            )
