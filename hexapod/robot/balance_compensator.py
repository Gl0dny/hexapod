...
class BalanceCompensator:
    ...
    def _apply_compensation(self, roll: float, pitch: float, yaw: float) -> None:
        ...
        try:
            ...
            roll_deg = np.clip(
                roll_deg, -self.max_compensation_angle, self.max_compensation_angle
            )
            pitch_deg = np.clip(
                pitch_deg, -self.max_compensation_angle, self.max_compensation_angle
            )
            yaw_deg = np.clip(
                yaw_deg, -self.max_compensation_angle, self.max_compensation_angle
            )
            ...
            if (
                abs(roll_deg) < 1.0 and abs(pitch_deg) < 1.0 and gyro_magnitude < 5.0
            ):  # 5.0 degrees/second threshold for movement
                # Gradually return to zero
                target_roll = self.last_compensation[0] * 0.8  # Reduce by 20%
                target_pitch = self.last_compensation[1] * 0.8
                print(
                    f"Returning to stable position: roll={target_roll:.1f}°, pitch={target_pitch:.1f}°"
                )
            ...
            # For significant tilts or movement, apply proportional compensation
                ...
                abs(roll_deg) > self.min_movement_threshold
                or abs(pitch_deg) > self.min_movement_threshold
                or gyro_magnitude > 5.0
            ):  # Only compensate if actually moving
                print(
                    f"Applying compensation: roll={roll_deg:.1f}°, pitch={pitch_deg:.1f}°, yaw={yaw_deg:.1f}°"
                )
                self.hexapod.move_body(
                    tx=0.0, ty=0.0, tz=0.0, roll=roll_deg, pitch=pitch_deg, yaw=yaw_deg
                )
