...
class IntentDispatcher:
    ...
    def handle_set_brightness(self, slots: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.exception(
                "No brightness_percentage provided for set_brightness command."
            )
        except ValueError:
            logger.exception(
                f"Invalid brightness_percentage value: {brightness_percentage}."
            )
    ...
    def handle_march_in_place(self, slots: Dict[str, Any]) -> None:
        ...
        try:
            ...
            duration_seconds = (
                self._parse_duration_in_seconds(march_time, time_unit)
                if march_time and time_unit
                else None
            )
        ...
        try:
            ...
            if "move_cycles" in slots:
                try:
                    ...
                    logger.exception(
                        f"Invalid move_cycles value: {slots['move_cycles']}"
                    )
            ...
            duration_seconds = (
                self._parse_duration_in_seconds(move_time, time_unit)
                if move_time and time_unit
                else None
            )
    ...
    def handle_rotate(self, slots: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if turn_direction and turn_direction not in valid_turn_directions:
                logger.exception(
                    f"Invalid turn_direction '{turn_direction}' for rotate command."
                )
                return
            ...
            if "rotate_angle" in slots:
                try:
                    ...
                    logger.exception(
                        f"Invalid rotate_angle value: {slots['rotate_angle']}"
                    )
            if "rotate_cycles" in slots:
                try:
                    ...
                    logger.exception(
                        f"Invalid rotate_cycles value: {slots['rotate_cycles']}"
                    )
            if "rotate_time" in slots:
                try:
                    ...
                    logger.exception(
                        f"Invalid rotate_time value: {slots['rotate_time']}"
                    )
            if "time_unit" in slots:
                time_unit = slots["time_unit"]

            duration_seconds = (
                self._parse_duration_in_seconds(rotate_time, time_unit)
                if rotate_time and time_unit
                else None
            )
            ...
            if angle is not None:
                ...
                self.task_interface.rotate(
                    turn_direction=turn_direction, cycles=rotate_cycles
                )
            elif duration_seconds is not None:
                self.task_interface.rotate(
                    turn_direction=turn_direction, duration=duration_seconds
                )
            else:
