...
class Recorder:
    ...
    def start_recording(self, filename: str = None, duration: Optional[float] = None) -> str:
        ...
        if self.is_recording:
            logger.info("Recording already in progress - saving current recording and starting new one")
            self.stop_recording()
        ...
        if duration and duration > 0:
            ...
            logger.info(f"Started recording to: {self.recording_base_filename}.wav (auto-stop in {duration}s)")
        else:
            self.recording_timer = None
            logger.info(f"Started continuous recording to: {self.recording_base_filename}_001.wav (15-min audio records)")
    ...
    def add_audio_frame(self, audio_data: bytes) -> None:
        ...
        if self.is_continuous_recording:
            ...
            audio_record_duration = current_time - self.recording_audio_record_start_time
            
            if audio_record_duration >= self.AUDIO_RECORD_DURATION:
                logger.info(f"Audio record {self.recording_audio_record_number} duration reached ({audio_record_duration:.1f}s), saving...")
    ...
    def _save_recording_audio_record(self) -> str:
        ...
        try:
            ...
            logger.user_info(f"Audio record {self.recording_audio_record_number} saved: {recording_path} (Duration: {audio_record_duration:.1f}s, Size: {file_size_str})")
    ...
    def stop_recording(self) -> str:
        ...
        if self.recording_frames:
            if self.is_continuous_recording:
                ...
                try:
                    recording_path = self.recordings_dir / f"{self.recording_base_filename}.wav"
                    ...
                    if file_size_mb >= 1024:
                        ...
                        file_size_str = f"{file_size_mb:.1f} MB"
                    logger.user_info(f"Recording finished and saved: {recording_path} (Duration: {total_duration:.1f}s, Size: {file_size_str})")
        ...
        if self.is_continuous_recording:
            ...
            logger.user_info(f"Continuous recording finished. Total duration: {total_duration:.1f}s, Total audio records: {total_audio_records}")
