...
def test_picovoice_on_file(input_file: Path, access_key: str):
    ...
    keyword_path = (
        ...
        / "kws"
        / "porcupine"
        / "hexapod_en_raspberry-pi_v3_0_0.ppn"
    )
    ...
    porcupine = pvporcupine.create(
        access_key=access_key, keyword_paths=[str(keyword_path)]
    )
    ...
    while True:
        ...
        if keyword_index >= 0:
            ...
            print(
                f"Keyword 'hexapod' detected at frame {wav_file.tell() // (frame_length * 2)}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Test Picovoice keyword spotting on a WAV file using the hexapod keyword model"
    )
    parser.add_argument("input_file", type=str, help="Path to the input WAV file")
    parser.add_argument(
        "--access-key", type=str, required=True, help="Picovoice access key"
    )

    args = parser.parse_args()

    test_picovoice_on_file(Path(args.input_file), args.access_key)
