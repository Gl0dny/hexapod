...
class TestJoint:
    def test_set_angle_within_limits(self, joint_fixture):
        ...
        joint_fixture.controller.set_speed.assert_called_with(
            joint_fixture.channel, Joint.DEFAULT_SPEED
        )
        joint_fixture.controller.set_acceleration.assert_called_with(
            joint_fixture.channel, Joint.DEFAULT_ACCEL
        )
    ...
    def test_angle_to_servo_target(self, joint_fixture):
        ...
        expected = int(
            joint_fixture.servo_min
            + (joint_fixture.servo_max - joint_fixture.servo_min)
            * (
                (0 - joint_fixture.angle_min)
                / (joint_fixture.angle_max - joint_fixture.angle_min)
            )
        )
