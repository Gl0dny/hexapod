...
# Add the hexapod directory to the Python path
hexapod_path = str(Path(__file__).parent.parent.parent.parent / "hexapod")
sys.path.append(hexapod_path)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test IMU readings for different orientations"
    )
    parser.add_argument(
        "--duration",
        type=float,
    ...
    try:
        ...
        try:
            while True:
                ...
                print(
                    f"\rAccel: X={accel_x:+.2f} Y={accel_y:+.2f} Z={accel_z:+.2f} | "
