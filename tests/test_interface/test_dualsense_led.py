...
from interface.controllers.gamepad_led_controllers.gamepad_led_controller import GamepadLEDColor, BaseGamepadLEDController
...
def test_breathing_animation(led_controller):
    ...
    success = led_controller.breathing_animation(GamepadLEDColor.WHITE, duration=2.0, cycles=2)
    if success:
    ...
    success = led_controller.breathing_animation(GamepadLEDColor.ORANGE, duration=1.5, cycles=1)
    if success:
        print("Breathing animation started")
...
def test_animation_stop(led_controller):
    ...
    success = led_controller.pulse(GamepadLEDColor.RED, duration=0.5, cycles=0)  # 0 = infinite
    if success:
...
def test_error_handling(led_controller):
    ...
    led_controller.set_color(GamepadLEDColor.BLUE, brightness=2.0)  # Should clamp to 1.0
