...
class TestODASDoASSLProcessor:
    ...
    def sample_tracked_data(self):
        ...
        return {"id": 1, "x": 1.0, "y": 0.0, "z": 0.0, "activity": 0.8}

    @pytest.fixture
    def sample_potential_data(self):
        """Sample potential source data."""
        return {"x": 0.5, "y": 0.5, "z": 0.0, "activity": 0.6}
    ...
    def test_data_manager_setup_failure(self, mock_lights_handler):
        ...
        with patch("hexapod.odas.odas_doa_ssl_processor.get_custom_logger"):
            ...
            processor.data_manager.odas_logs_dir = Path(
    ...
    def test_get_tracked_sources_azimuths(
        self, mock_lights_handler, sample_tracked_data
    ):
    ...
    def test_print_debug_info_with_sources(
        self, mock_lights_handler, capsys, sample_tracked_data
    ):
    ...
    def test_print_debug_info_disabled(
        self, mock_lights_handler, capsys, sample_tracked_data
    ):
    ...
    def test_process_json_data_potential(
        self, mock_lights_handler, sample_potential_data
    ):
    ...
    def test_process_json_data_limit_to_top_4(self, mock_lights_handler):
        ...
        with patch("hexapod.odas.odas_doa_ssl_processor.get_custom_logger"):
            ...
            json_data = "".join(json.dumps(data) for data in sources_data).encode(
                "utf-8"
            )
            processor._process_json_data(json_data, "tracked", mock_file)
            ...
            assert (
                abs(max(activities) - 0.6) < 1e-10
            )  # Highest activity (account for floating point precision)
            assert (
                abs(min(activities) - 0.3) < 1e-10
            )  # 4th highest activity (account for floating point precision)
    ...
    def test_process_json_data_exception(self, mock_lights_handler, caplog):
        ...
        with patch("hexapod.odas.odas_doa_ssl_processor.get_custom_logger"):
            ...
            processor.lights_handler.animation.update_sources.side_effect = Exception(
                "Test error"
            )

            json_data = json.dumps(
                {"id": 1, "x": 1.0, "y": 0.0, "z": 0.0, "activity": 0.8}
            ).encode("utf-8")
    ...
    def test_start_odas_process_success(self, mock_popen, mock_lights_handler):
        ...
        with patch("hexapod.odas.odas_doa_ssl_processor.get_custom_logger"):
            ...
            with patch(
                "hexapod.odas.odas_doa_ssl_processor.Path.exists", return_value=True
            ):
    ...
    def test_start_odas_process_config_not_found(self, mock_popen, mock_lights_handler):
        ...
        with patch("hexapod.odas.odas_doa_ssl_processor.get_custom_logger"):
            ...
            with patch(
                "hexapod.odas.odas_doa_ssl_processor.Path.exists", return_value=False
            ):
                processor.start_odas_process()
        ...
        with patch("hexapod.odas.odas_doa_ssl_processor.get_custom_logger"):
            ...
            with patch.object(processor, "start_server") as mock_start_server:
                ...
                with patch(
                    ...
                    with patch.object(
                        processor, "accept_and_handle_data"
                    ) as mock_accept:
                        processor.start()
    ...
    def test_data_manager_log_file_creation_error(self, mock_lights_handler, caplog):
        ...
        with patch("hexapod.odas.odas_doa_ssl_processor.get_custom_logger"):
            ...
            with patch(
                "hexapod.odas.odas_doa_ssl_processor.open",
                ...
                processor.data_manager.tracked_log = (
                    processor.data_manager._open_log_file("test.log")
                )
                processor.data_manager.potential_log = (
                    processor.data_manager._open_log_file("test2.log")
                )
    ...
    def test_gui_manager_handle_disconnection_success(
        self, mock_lights_handler, caplog
    ):
        ...
        with patch("hexapod.odas.odas_doa_ssl_processor.get_custom_logger"):
            ...
            with (
                patch.object(gui_manager, "connect") as mock_connect,
    ...
    def test_gui_manager_handle_disconnection_failure(
        self, mock_lights_handler, caplog
    ):
        ...
        with patch("hexapod.odas.odas_doa_ssl_processor.get_custom_logger"):
            ...
            with (
                patch.object(
                    gui_manager, "connect", side_effect=Exception("Connection failed")
                ),
                patch.object(gui_manager, "_close_sockets") as mock_close,
            ):
    ...
    def test_start_odas_process_with_config_read_error(
        self, mock_lights_handler, caplog
    ):
        ...
        with patch("hexapod.odas.odas_doa_ssl_processor.get_custom_logger"):
            ...
            with (
                ...
                patch(
                    "hexapod.odas.odas_doa_ssl_processor.Path.read_text",
    ...
    def test_monitor_odas_output_with_process_termination(
        self, mock_lights_handler, caplog
    ):
    ...
    def test_start_method_with_odas_process_creation_error(
        self, mock_lights_handler, caplog
    ):
    ...
    def test_start_method_with_gui_connection_disabled(self, mock_lights_handler):
        ...
        with patch("hexapod.odas.odas_doa_ssl_processor.get_custom_logger"):
            ...
            with (
                patch.object(processor, "start_server") as mock_start_server,
    ...
    def test_process_json_data_with_zero_activity_source(self, mock_lights_handler):
        ...
        with patch("hexapod.odas.odas_doa_ssl_processor.get_custom_logger"):
            ...
            json_data = json.dumps(
                {"id": 1, "x": 1.0, "y": 0.0, "z": 0.0, "activity": 0.0}
            ).encode("utf-8")
    ...
    def test_get_tracked_sources_azimuths_with_mixed_coordinates(
        self, mock_lights_handler
    ):
    ...
    def test_data_manager_setup_with_existing_directories(
        self, mock_lights_handler, tmp_path
    ):
