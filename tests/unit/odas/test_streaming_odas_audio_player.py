...
class TestStreamingODASAudioPlayer:
    ...
    def test_init_default_parameters(self, tmp_path):
        ...
        with patch(
            "hexapod.odas.streaming_odas_audio_player.paramiko.SSHClient"
        ) as mock_ssh_class:
    ...
    def test_connect_ssh_with_key(self, mock_ssh_client):
        ...
        mock_ssh_client.connect.assert_called_once_with(
            player.remote_host, username=player.remote_user, key_filename="/test/key"
        )
    ...
    def test_connect_ssh_without_key(self, mock_ssh_client):
        ...
        mock_ssh_client.connect.assert_called_once_with(
            player.remote_host, username=player.remote_user
        )
    ...
    def test_convert_to_wav_success(self, tmp_path):
        ...
        try:
            ...
            assert (
                "sox" in str(e).lower() or "command not found" in str(e).lower()
            ), f"Expected sox-related error, got: {e}"
    ...
    def test_start_audio_stream(self, mock_output_stream, mock_audio_stream):
        ...
        mock_output_stream.assert_called_once_with(
            samplerate=44100, channels=1, callback=player.audio_callback, blocksize=1024
        )
    ...
    @patch("hexapod.odas.streaming_odas_audio_player.time.sleep")
    def test_stream_audio_success(
        self, mock_sleep, mock_ssh_client, mock_sftp, tmp_path
    ):
        ...
        assert (
            len(sleep_calls) >= 1
        ), f"Expected sleep(0.5) call, but got calls: {mock_sleep.call_args_list}"
    ...
    def test_monitor_files_success(self, mock_thread, mock_ssh_client):
        ...
        mock_thread.assert_called_once_with(
            target=player.stream_audio, args=("postfiltered",), daemon=True
        )
