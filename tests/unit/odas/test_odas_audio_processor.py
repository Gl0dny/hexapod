...
class TestODASAudioProcessor:
    ...
    def test_convert_audio_sample_accumulation(self, processor_default):
        ...
        small_data = b'\x00\x01' * 500  # 500 samples = 125 samples per channel, need more
    ...
    def test_convert_audio_channel_selection(self, processor_custom, sample_audio_data_custom):
    ...
    def test_convert_audio_padding(self, processor_default):
        ...
        with patch('resampy.resample') as mock_resample:
            mock_resample.return_value = np.array([1, 2, 3], dtype=np.int16)  # Very short
    ...
    def test_read_audio_file_not_exists(self, mock_stat, mock_exists, mock_file, processor_default):
    ...
    @patch('pathlib.Path.exists')
    @patch('pathlib.Path.stat')
    def test_read_audio_no_new_data(self, mock_stat, mock_exists, mock_file, processor_default):
    ...
    @patch('pathlib.Path.exists')
    @patch('pathlib.Path.stat')
    def test_read_audio_incomplete_frame(self, mock_stat, mock_exists, mock_file, processor_default):
    ...
    @patch('pathlib.Path.exists')
    @patch('pathlib.Path.stat')
    def test_read_audio_multiple_frames(self, mock_stat, mock_exists, mock_file, processor_default, sample_audio_data):
        ...
        bytes_per_frame = 2 * processor_default.source_channels * processor_default.frame_length
    ...
    @patch('pathlib.Path.exists')
    @patch('pathlib.Path.stat')
    def test_read_audio_file_error(self, mock_stat, mock_exists, mock_file, processor_default):
