...
class TestGaitGenerator:
    ...
    def test_create_gait_tripod(self, gait_generator, mock_tripod_gait):
        ...
        with patch(
            "hexapod.gait_generator.gait_generator.TripodGait",
            return_value=mock_tripod_gait,
        ):
            gait_generator.create_gait(
                "tripod", step_radius=25.0, leg_lift_distance=15.0
            )
    ...
    def test_create_gait_wave(self, gait_generator, mock_wave_gait):
        ...
        with patch(
            "hexapod.gait_generator.gait_generator.WaveGait",
            return_value=mock_wave_gait,
        ):
    ...
    @patch("time.sleep")
    def test_execute_phase_swing_and_stance_legs(
        self, mock_sleep, gait_generator, mock_tripod_gait
    ):
        ...
        for leg_idx in swing_legs:
            mock_tripod_gait.calculate_leg_target.assert_any_call(
                leg_idx, is_swing=True
            )
            mock_tripod_gait.calculate_leg_path.assert_any_call(
                leg_idx, Vector3D(0, 0, 0), is_swing=True
            )
        ...
        for leg_idx in stance_legs:
            ...
            mock_tripod_gait.calculate_leg_path.assert_any_call(
                leg_idx, Vector3D(0, 0, 0), is_swing=False
            )
    ...
    def test_execute_phase_no_current_gait(self, gait_generator):
        ...
        assert (
            not hasattr(gait_generator.current_gait, "calculate_leg_target")
            or gait_generator.current_gait is None
        )
    ...
    def test_execute_waypoints_simultaneous_movement(self, mock_sleep, gait_generator):
        ...
        swing_paths = {
            ...
            2: Mock(
                waypoints=[Vector3D(0, 0, 0), Vector3D(10, 0, 0), Vector3D(20, 0, 0)]
            ),
        }
        stance_paths = {
        ...
        gait_generator._execute_waypoints(
            swing_legs, swing_paths, stance_legs, stance_paths
        )
    ...
    def test_execute_full_cycle_no_current_state(
        self, gait_generator, mock_tripod_gait
    ):
    ...
    @patch("time.sleep")
    def test_run_gait_loop_max_cycles(
        self, mock_sleep, gait_generator, mock_tripod_gait
    ):
    ...
    @patch("time.sleep")
    def test_run_gait_loop_max_duration(
        self, mock_sleep, gait_generator, mock_tripod_gait
    ):
    ...
    @patch("time.sleep")
    def test_run_gait_loop_stop_event(
        self, mock_sleep, gait_generator, mock_tripod_gait
    ):
    ...
    def test_start_no_gait(self, gait_generator):
        ...
        with pytest.raises(
            ValueError, match="No current gait set. Call create_gait\\(\\) first."
        ):
            gait_generator.start()
    ...
    def test_get_cycle_statistics(self, gait_generator):
        ...
        assert stats == {"total_cycles": 5, "total_phases": 15, "is_running": True}
    ...
    def test_calculate_cycles_for_angle(self, gait_generator):
        ...
        with patch.object(
            gait_generator, "_calculate_rotation_per_cycle", return_value=30.0
        ):
            cycles = gait_generator._calculate_cycles_for_angle(
                angle_degrees, step_radius
            )
    ...
    def test_execute_rotation_by_angle(self, gait_generator, mock_tripod_gait):
        ...
        with patch.object(
            gait_generator, "_calculate_cycles_for_angle", return_value=3
        ):
            with patch.object(gait_generator, "execute_cycles") as mock_execute:
                gait_generator.execute_rotation_by_angle(
                    angle_degrees, rotation_direction, step_radius
                )

                mock_tripod_gait.set_direction.assert_called_once_with(
    ...
    @patch("time.sleep")
    def test_return_legs_to_neutral_tripod(
        self, mock_sleep, gait_generator, mock_tripod_gait
    ):
    ...
    @patch("time.sleep")
    def test_return_legs_to_neutral_wave(
        self, mock_sleep, gait_generator, mock_wave_gait
    ):
    ...
    def test_return_legs_to_neutral_no_gait(self, gait_generator):
        ...
        with patch("hexapod.gait_generator.gait_generator.logger") as mock_logger:
            ...
            mock_logger.warning.assert_called_with(
                "No current gait or hexapod, cannot return legs to neutral."
            )
    ...
    def test_queue_direction_same_as_current(self, gait_generator, mock_tripod_gait):
        ...
        mock_tripod_gait.DIRECTION_MAP = {
            "forward": (0, 1)
        }  # Different from current (1, 0)
    ...
    @patch("time.sleep")
    def test_run_gait_loop_direction_changes(
        self, mock_sleep, gait_generator, mock_tripod_gait
    ):
        ...
        with patch.object(
            gait_generator, "_execute_full_cycle", side_effect=mock_execute
        ):
    ...
    def test_run_gait_loop_exception_handling(self, gait_generator, mock_tripod_gait):
        ...
        with patch.object(
            gait_generator, "_execute_full_cycle", side_effect=Exception("Test error")
        ):
            with pytest.raises(Exception, match="Test error"):
                gait_generator._run_gait_loop()

    def test_run_gait_loop_stop_requested_during_cycle(
        self, gait_generator, mock_tripod_gait
    ):
