...
class TestCalibration:
    ...
    def test_calibrate_all_servos_success(
        self, mock_input_handler_class, mock_rename_thread, calibration
    ):
        ...
        with (
            patch.object(
                calibration, "_check_zero_angle", return_value=True
            ) as mock_check_zero,
            ...
            patch.object(
                calibration, "_calibrate_servo_min_inverted"
            ) as mock_calibrate_min_inv,
            patch.object(
                calibration, "_calibrate_servo_max_inverted"
            ) as mock_calibrate_max_inv,
        ...
        mock_rename_thread.assert_called_once_with(
            mock_input_handler, "CalibrationInputHandler"
        )

        # Verify hexapod methods were called
        assert (
            calibration.hexapod.move_to_position.call_count >= 2
    ...
    @patch("hexapod.interface.NonBlockingConsoleInputHandler")
    def test_calibrate_all_servos_with_stop_event(
        self, mock_input_handler_class, calibration
    ):
    ...
    def test_calibrate_servo_max(self, calibration):
        ...
        with patch.object(calibration, "_calibrate_joint") as mock_calibrate:
            ...
            mock_calibrate.assert_called_once_with(
                0, "coxa", "angle_max", 90.0, "servo_max", stop_event=None, invert=False
            )
    ...
    def test_calibrate_servo_min_inverted(self, calibration):
        ...
        with patch.object(calibration, "_calibrate_joint") as mock_calibrate:
            ...
            mock_calibrate.assert_called_once_with(
                0, "coxa", "angle_min", -90.0, "servo_max", stop_event=None, invert=True
            )
    ...
    def test_calibrate_servo_max_inverted(self, calibration):
        ...
        with patch.object(calibration, "_calibrate_joint") as mock_calibrate:
            ...
            mock_calibrate.assert_called_once_with(
                0, "coxa", "angle_max", 90.0, "servo_min", stop_event=None, invert=True
            )
    ...
    def test_validate_servo_input_inverted_min_too_high(self, calibration):
        ...
        assert (
            calibration._validate_servo_input("servo_min", 2000, invert=True) is False
        )
    ...
    def test_validate_servo_input_normal_min_too_high(self, calibration):
        ...
        assert (
            calibration._validate_servo_input("servo_min", 2000, invert=False) is False
        )

    def test_validate_servo_input_normal_max_too_low(self, calibration):
        """Test normal servo input validation with max too low."""
        assert (
            calibration._validate_servo_input("servo_max", 992, invert=False) is False
        )
    ...
    def test_calibrate_joint_success(self, mock_print, calibration):
        ...
        with (
            patch.object(
                calibration, "_prompt_servo_value", return_value=1500
            ) as mock_prompt,
            patch.object(
                calibration, "_confirm_calibration_done", return_value=True
            ) as mock_confirm,
    ...
    def test_calibrate_joint_stop_event(self, calibration):
        ...
        with patch.object(calibration, "_prompt_servo_value", return_value=None):
            calibration._calibrate_joint(
                0, "coxa", "angle_min", -90.0, "servo_min", stop_event
            )
    ...
    def test_calibrate_joint_invalid_input(self, calibration):
        ...
        with (
            ...
            patch.object(
                calibration, "_validate_servo_input", side_effect=[False, True]
            ) as mock_validate,
    ...
    def test_calibrate_joint_servo_max_less_than_min(self, calibration):
        ...
        with (
            ...
            patch.object(
                calibration, "_validate_servo_input", return_value=True
            ) as mock_validate,
            patch.object(
                calibration, "_confirm_calibration_done", side_effect=[False, True]
            ) as mock_confirm,
            patch.object(
                calibration, "_calibrate_servo", side_effect=fix_servo_values
            ) as mock_calibrate,
        ):
    ...
    def test_calibrate_joint_exception(self, calibration):
        ...
        with (
            patch.object(
                calibration,
                "_prompt_servo_value",
                side_effect=[Exception("Test error"), 1500],
            ) as mock_prompt,
            patch.object(
                calibration, "_validate_servo_input", return_value=True
            ) as mock_validate,
    ...
    def test_calibrate_servo_exception(self, calibration):
        ...
        calibration.hexapod.legs[0].coxa.update_calibration.side_effect = Exception(
            "Test error"
        )
    ...
    def test_joint_constants_usage(self, calibration):
        ...
        assert (
            calibration._validate_servo_input("servo_min", Joint.SERVO_INPUT_MIN)
            is True
        )
        assert (
            calibration._validate_servo_input("servo_max", Joint.SERVO_INPUT_MAX)
            is True
        )
        assert (
            calibration._validate_servo_input("servo_min", Joint.SERVO_INPUT_MIN - 1)
            is False
        )
        assert (
            calibration._validate_servo_input("servo_max", Joint.SERVO_INPUT_MAX + 1)
            is False
        )
