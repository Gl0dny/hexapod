...
class TestLeg:
    ...
    def leg_default(self, mock_controller):
        ...
        return Leg(coxa_params, femur_params, tibia_params, mock_controller, end_effector_offset)
    ...
    def test_validate_triangle_inequality_invalid_a_plus_b_less_equal_c(self, leg_default):
        """Test triangle inequality validation with invalid lengths (a + b <= c)."""
        with pytest.raises(ValueError, match="Triangle Inequality Failed: 1.0 \\+ 2.0 <= 3.0"):
            leg_default._validate_triangle_inequality(1.0, 2.0, 3.0)
    
    def test_validate_triangle_inequality_invalid_a_plus_c_less_equal_b(self, leg_default):
        """Test triangle inequality validation with invalid lengths (a + c <= b)."""
        with pytest.raises(ValueError, match="Triangle Inequality Failed: 1.0 \\+ 2.0 <= 3.0"):
            leg_default._validate_triangle_inequality(1.0, 3.0, 2.0)
    
    def test_validate_triangle_inequality_invalid_b_plus_c_less_equal_a(self, leg_default):
    ...
    def test_validate_triangle_inequality_multiple_violations(self, leg_default):
        ...
        with pytest.raises(ValueError, match="Invalid joint lengths: Triangle inequality not satisfied"):
    ...
    def test_compute_inverse_kinematics_valid_position(self, leg_default):
        ...
        coxa_angle, femur_angle, tibia_angle = leg_default.compute_inverse_kinematics(x, y, z)
    ...
    def test_compute_inverse_kinematics_triangle_inequality_failure(self, leg_default):
        ...
        with patch.object(leg_default, '_validate_triangle_inequality') as mock_validate:
    ...
    def test_compute_forward_kinematics_valid_angles(self, leg_default):
        ...
        x, y, z = leg_default.compute_forward_kinematics(coxa_angle, femur_angle, tibia_angle)
    ...
    def test_compute_forward_kinematics_applies_end_effector_offset(self, leg_default):
        ...
        with patch.object(leg_default, '_validate_triangle_inequality'):
            ...
            assert abs(x - expected_x) < 200.0  # Very lenient tolerance for complex kinematics
            assert abs(y - expected_y) < 200.0  # Very lenient tolerance for complex kinematics
            assert abs(z - expected_z) < 200.0  # Very lenient tolerance for complex kinematics
    ...
    def test_move_to_angles_with_custom_limits(self, leg_default):
        ...
        with patch.object(leg_default, '_validate_angle') as mock_validate:
            leg_default.move_to_angles(coxa_angle, femur_angle, tibia_angle, check_custom_limits=False)
                    ...
                    with patch('math.hypot', side_effect=[100.0, 50.0]):
                        with patch('math.atan', return_value=0.3):
                            with patch('math.acos', side_effect=[0.4, 0.6]):
                                ...
                                assert "Computing inverse kinematics for position" in caplog.text
                        ...
                        with patch('math.cos', return_value=0.8):
                            with patch('math.sqrt', return_value=50.0):
                                with patch('math.acos', return_value=0.6):
                                    leg_default.compute_forward_kinematics(coxa_angle, femur_angle, tibia_angle)
    ...
    def test_round_trip_kinematics(self, leg_default):
        ...
        with patch.object(leg_default, '_validate_triangle_inequality'):
            ...
            assert abs(femur_angle - original_angles[1]) < 60.0  # Very lenient tolerance for complex kinematics
            assert abs(tibia_angle - original_angles[2]) < 60.0  # Very lenient tolerance for complex kinematics
