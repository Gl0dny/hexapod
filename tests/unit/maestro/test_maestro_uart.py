...
class TestMaestroUART:
    ...
    def maestro_uart(self, mock_serial):
        ...
        with patch(
            "hexapod.maestro.maestro_uart.serial.Serial", return_value=mock_serial
        ):
    ...
    def test_init_logging(self, mock_serial, caplog):
        ...
        with patch(
            ...
            assert (
                "MaestroUART initialized successfully with device=/dev/ttyS0, baudrate=9600"
                in caplog.text
            )
    ...
    def test_set_multiple_targets_sequential(self, maestro_uart, mock_serial, caplog):
        ...
        expected_command = bytes(
            [
                ...
                0x1F,  # Command header
                3,
                3,  # Number of targets, first channel
                0 & 0x7F,
                (0 >> 7) & 0x7F,  # Target 0
                6000 & 0x7F,
                (6000 >> 7) & 0x7F,  # Target 6000
                8000 & 0x7F,
                (8000 >> 7) & 0x7F,  # Target 8000
            ]
        )
        mock_serial.write.assert_called_once_with(expected_command)
        assert "Multiple targets set: [(3, 0), (4, 6000), (5, 8000)]" in caplog.text

    def test_set_multiple_targets_single_channel(
        self, maestro_uart, mock_serial, caplog
    ):
        ...
        expected_command = bytes(
            [
                ...
                0x1F,  # Command header
                1,
                5,  # Number of targets, first channel
                4000 & 0x7F,
                (4000 >> 7) & 0x7F,  # Target 4000
            ]
        )
        mock_serial.write.assert_called_once_with(expected_command)
    ...
    def test_high_target_value_handling(self, maestro_uart, mock_serial):
        ...
        expected_command = bytes(
            [0xAA, 0x0C, 0x04, 0, 20000 & 0x7F, (20000 >> 7) & 0x7F]
        )
        mock_serial.write.assert_called_once_with(expected_command)
    ...
    def test_multiple_targets_large_values(self, maestro_uart, mock_serial, caplog):
        ...
        expected_command = bytes(
            [
                ...
                0x1F,  # Command header
                2,
                0,  # Number of targets, first channel
                20000 & 0x7F,
                (20000 >> 7) & 0x7F,  # Target 20000
                30000 & 0x7F,
                (30000 >> 7) & 0x7F,  # Target 30000
            ]
        )
        mock_serial.write.assert_called_once_with(expected_command)
    ...
    def test_serial_communication_failure_handling(self, maestro_uart, mock_serial):
        ...
        mock_serial.read.side_effect = [
            b"\x01",
            b"\x00",
        ]  # First byte, then empty second byte
