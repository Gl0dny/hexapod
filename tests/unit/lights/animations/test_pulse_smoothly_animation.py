...
class TestPulseSmoothlyAnimation:
    ...
    def test_init_default_parameters(self, mock_lights):
        ...
        with patch(
            "hexapod.lights.animations.pulse_smoothly_animation.Animation.__init__"
        ) as mock_super_init:
    ...
    def test_execute_animation_forward_interpolation(
        ...
        for call in calls:
            ...
            for i in range(3):
                assert (
                    base_rgb[i] <= rgb[i] <= pulse_rgb[i]
                    or pulse_rgb[i] <= rgb[i] <= base_rgb[i]
                )

    def test_execute_animation_backward_interpolation(
        self, animation_default, mock_lights
    ):
    ...
    def test_execute_animation_stop_during_forward(
        self, animation_default, mock_lights
    ):
    ...
    def test_execute_animation_stop_during_backward(
        self, animation_default, mock_lights
    ):
    ...
    def test_execute_animation_interpolation_calculation(
        self, animation_default, mock_lights
    ):
    ...
    def test_execute_animation_interpolation_steps(
        self, animation_default, mock_lights
    ):
    ...
    def test_execute_animation_different_speeds(self, mock_lights):
        ...
        animation_fast = PulseSmoothlyAnimation(lights=mock_lights, pulse_speed=0.01)
        ...
        animation_slow = PulseSmoothlyAnimation(lights=mock_lights, pulse_speed=1.0)
    ...
    def test_execute_animation_rgb_conversion(self, animation_default, mock_lights):
        ...
        assert (
            call[0][0] == ColorRGB.BLUE.rgb
        )  # Should be RGB tuple, not ColorRGB object
    ...
    def test_interpolation_range_forward(self, animation_default, mock_lights):
        ...
        expected_rgb = (
            int(
                ColorRGB.BLUE.rgb[0]
                + (ColorRGB.GREEN.rgb[0] - ColorRGB.BLUE.rgb[0]) * 95 / 100
            ),
            int(
                ColorRGB.BLUE.rgb[1]
                + (ColorRGB.GREEN.rgb[1] - ColorRGB.BLUE.rgb[1]) * 95 / 100
            ),
            int(
                ColorRGB.BLUE.rgb[2]
                + (ColorRGB.GREEN.rgb[2] - ColorRGB.BLUE.rgb[2]) * 95 / 100
            ),
        )
    ...
    def test_interpolation_range_backward(self, animation_default, mock_lights):
        ...
        backward_start = mock_lights.set_color_rgb.call_args_list[
            20
        ]  # First backward call
        assert (
            backward_start[0][0] == ColorRGB.GREEN.rgb
        )  # Should be pulse color (i=100)
        ...
        expected_rgb = (
            int(
                ColorRGB.BLUE.rgb[0]
                + (ColorRGB.GREEN.rgb[0] - ColorRGB.BLUE.rgb[0]) * 5 / 100
            ),
            int(
                ColorRGB.BLUE.rgb[1]
                + (ColorRGB.GREEN.rgb[1] - ColorRGB.BLUE.rgb[1]) * 5 / 100
            ),
            int(
                ColorRGB.BLUE.rgb[2]
                + (ColorRGB.GREEN.rgb[2] - ColorRGB.BLUE.rgb[2]) * 5 / 100
            ),
        )
