...
def animation_custom(mock_lights):
    ...
    return OppositeRotateAnimation(lights=mock_lights, interval=0.2, color=ColorRGB.RED)
...
class TestOppositeRotateAnimation:
    ...
    def test_init_default_parameters(self, mock_lights):
        ...
        with patch(
            "hexapod.lights.animations.opposite_rotate_animation.Animation.__init__"
        ) as mock_super_init:
    ...
    def test_init_custom_parameters(self, mock_lights):
        ...
        with patch(
            ...
            animation = OppositeRotateAnimation(
                lights=mock_lights, interval=0.2, color=ColorRGB.RED
            )
    ...
    def test_execute_animation_single_led_sequence(
    ...
    def test_execute_animation_direction_switching(
        self, animation_default, mock_lights
    ):
    ...
    def test_execute_animation_single_led(self, mock_lights):
        ...
        assert mock_lights.clear.call_count == 2
        assert (
            mock_lights.set_color.call_count == 2
        )  # Two calls due to the while loop structure
        call = mock_lights.set_color.call_args_list[0]
        assert call[1]["led_index"] == 0
    ...
    def test_execute_animation_max_offset_calculation(
        self, animation_default, mock_lights
    ):
    ...
    def test_execute_animation_direction_switching_logic(self, animation_default):
        ...
        animation_default.direction = (
            OppositeRotateAnimation.BACKWARD
            if animation_default.direction == OppositeRotateAnimation.FORWARD
