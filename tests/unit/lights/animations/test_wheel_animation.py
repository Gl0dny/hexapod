...
def animation_custom(mock_lights):
    ...
    return WheelAnimation(
        lights=mock_lights, use_rainbow=False, color=ColorRGB.RED, interval=0.1
    )
...
class TestWheelAnimation:
    ...
    def test_init_default_parameters(self, mock_lights):
        ...
        with patch(
            "hexapod.lights.animations.wheel_animation.Animation.__init__"
        ) as mock_super_init:
    ...
    def test_init_validation_error(self, mock_lights):
        ...
        with (
            patch("hexapod.lights.animations.wheel_animation.Animation.__init__"),
            ...
            with pytest.raises(
                ...
                WheelAnimation(lights=mock_lights, use_rainbow=False, color=None)
    ...
    def test_execute_animation_clear_before_each_led(
        self, animation_default, mock_lights
    ):
        """Test that clear is called before each LED."""
        # Mock stop_event to allow a few LEDs
        animation_default.stop_event.wait = Mock(
