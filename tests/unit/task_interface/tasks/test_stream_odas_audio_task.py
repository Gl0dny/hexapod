...
class TestStreamODASAudioTask:
    ...
    def test_initialize_odas_processor(
        self, stream_audio_task, mock_lights_handler, mock_odas_processor
    ):
        """Test ODAS processor initialization."""
        with patch(
            "hexapod.task_interface.tasks.stream_odas_audio_task.time.sleep"
        ) as mock_sleep:
    ...
    def test_verify_ssh_connection_success(self, stream_audio_task):
        ...
        with (
            ...
            patch(
                "hexapod.task_interface.tasks.stream_odas_audio_task.paramiko.SSHClient"
            ) as mock_ssh_class,
        ):
    ...
    def test_verify_ssh_connection_port_closed(self, stream_audio_task):
        ...
        with (
            patch("socket.socket") as mock_socket,
            patch(
                "hexapod.task_interface.tasks.stream_odas_audio_task.logger"
            ) as mock_logger,
        ):
    ...
    def test_verify_ssh_connection_auth_failure(self, stream_audio_task):
        ...
        with (
            ...
            patch(
                "hexapod.task_interface.tasks.stream_odas_audio_task.paramiko.AuthenticationException",
                Exception,
            ) as mock_auth_exc,
            patch(
                "hexapod.task_interface.tasks.stream_odas_audio_task.logger"
    ...
    def test_start_remote_streaming_success(
        self, stream_audio_task, mock_odas_processor
    ):
        """Test successful remote streaming start."""
        with (
            patch.object(
                stream_audio_task, "_verify_ssh_connection", return_value=True
            ),
            patch(
                "hexapod.task_interface.tasks.stream_odas_audio_task.paramiko.SSHClient"
            ) as mock_ssh_class,
            ...
            assert (
                mock_ssh.exec_command.call_count >= 2
            )  # Check script + create log dir
    ...
    def test_start_remote_streaming_connection_failure(self, stream_audio_task):
        ...
        with (
            patch.object(
                stream_audio_task, "_verify_ssh_connection", return_value=False
            ),
            patch(
                "hexapod.task_interface.tasks.stream_odas_audio_task.logger"
            ) as mock_logger,
    ...
    def test_execute_task_success(
        self, stream_audio_task, mock_hexapod, mock_lights_handler, mock_odas_processor
    ):
        """Test successful execution of stream ODAS audio task."""
        with (
            patch.object(stream_audio_task, "_initialize_odas_processor"),
            patch.object(stream_audio_task, "_start_remote_streaming"),
            patch(
                "hexapod.task_interface.tasks.stream_odas_audio_task.threading.Thread"
    ...
    def test_execute_task_exception_handling(
        ...
        with (
            ...
            assert "ODAS audio streaming task failed" in str(
    ...
    def test_execute_task_cleanup_always_called(
        self, stream_audio_task, mock_hexapod, mock_lights_handler, mock_odas_processor
    ):
    ...
    def test_execute_task_threading_behavior(
        self, stream_audio_task, mock_hexapod, mock_lights_handler, mock_odas_processor
    ):
    ...
    def test_execute_task_wait_loop(
        self, stream_audio_task, mock_hexapod, mock_lights_handler, mock_odas_processor
    ):
