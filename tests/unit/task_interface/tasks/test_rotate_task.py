...
class TestRotateTask:
    ...
    def rotate_task_angle(self, mock_hexapod, mock_lights_handler):
        ...
        return RotateTask(mock_hexapod, mock_lights_handler, angle=90.0, turn_direction="clockwise")
    
    @pytest.fixture
    def rotate_task_cycles(self, mock_hexapod, mock_lights_handler):
        """Create RotateTask instance with cycles."""
        return RotateTask(mock_hexapod, mock_lights_handler, cycles=5, turn_direction="counterclockwise")
    
    @pytest.fixture
    def rotate_task_duration(self, mock_hexapod, mock_lights_handler):
        """Create RotateTask instance with duration."""
        return RotateTask(mock_hexapod, mock_lights_handler, duration=10.0, turn_direction="left")
    ...
    def test_perform_rotation_with_angle(self, rotate_task_angle, mock_hexapod):
        ...
        with patch('hexapod.task_interface.tasks.rotate_task.logger') as mock_logger:
            ...
            mock_hexapod.wait_until_motion_complete.assert_called_once_with(rotate_task_angle.stop_event)
            
            # Verify gait was created
            mock_hexapod.gait_generator.create_gait.assert_called_once_with("tripod", **mock_hexapod.gait_params["rotation"])
            
            # Verify direction was set
            mock_hexapod.gait_generator.current_gait.set_direction.assert_called_once_with("neutral", rotation=1.0)
            ...
            # Verify logging
            mock_logger.info.assert_any_call("Starting rotation with direction: clockwise")
            mock_logger.info.assert_any_call("Rotating 90.0 degrees using angle-based rotation")
            mock_logger.info.assert_any_call("Started rotation execution for 90.0 degrees in background thread")
    ...
    def test_perform_rotation_with_cycles(self, rotate_task_cycles, mock_hexapod):
        ...
        with patch('hexapod.task_interface.tasks.rotate_task.logger') as mock_logger:
            ...
            # Verify logging
            mock_logger.info.assert_any_call("Starting rotation with direction: counterclockwise")
            mock_logger.info.assert_any_call("Executing 5 rotation cycles")
            mock_logger.info.assert_any_call("Started execution of 5 rotation cycles in background thread")
    ...
    def test_perform_rotation_with_duration(self, rotate_task_duration, mock_hexapod):
        ...
        with patch('hexapod.task_interface.tasks.rotate_task.logger') as mock_logger:
            ...
            mock_logger.info.assert_any_call("Started duration-based rotation for 10.0 seconds in background thread")
    ...
    def test_rotation_direction_mapping(self, mock_hexapod, mock_lights_handler):
        ...
        for direction, expected_rotation in test_cases:
            task = RotateTask(mock_hexapod, mock_lights_handler, turn_direction=direction, cycles=1)
            
            with patch('hexapod.task_interface.tasks.rotate_task.logger'):
                task._perform_rotation()
                
                # Verify direction was set correctly
                mock_hexapod.gait_generator.current_gait.set_direction.assert_called_with("neutral", rotation=expected_rotation)
    ...
    def test_execute_task_success(self, rotate_task_angle, mock_hexapod, mock_lights_handler):
        """Test successful execution of rotate task."""
        with patch.object(rotate_task_angle, '_perform_rotation'), \
            ...
            mock_hexapod.wait_until_motion_complete.assert_called_with(rotate_task_angle.stop_event)
            
            # Verify logging
            mock_logger.info.assert_any_call("RotateTask started")
            mock_logger.info.assert_any_call("Performing rotation with direction: clockwise.")
            mock_logger.info.assert_any_call("RotateTask completed")
    
    def test_execute_task_exception_handling(self, rotate_task_angle, mock_hexapod, mock_lights_handler):
        """Test exception handling during rotate task execution."""
        # Make _perform_rotation raise an exception
        with patch.object(rotate_task_angle, '_perform_rotation', side_effect=Exception("Rotation failed")), \
             patch('hexapod.task_interface.tasks.rotate_task.logger') as mock_logger:
