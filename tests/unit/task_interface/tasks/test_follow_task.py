...
class TestFollowTask:
    ...
    def test_init_default_parameters(
        ...
        with patch("hexapod.task_interface.tasks.follow_task.logger") as mock_logger:
            ...
            assert (
                task.external_control_paused_event == mock_external_control_paused_event
            )
    ...
    def test_get_movement_direction_from_odas_no_sources(
        self, follow_task, mock_odas_processor
    ):
    ...
    def test_get_movement_direction_from_odas_right(
        self, follow_task, mock_odas_processor
    ):
    ...
    def test_get_movement_direction_from_odas_forward(
        self, follow_task, mock_odas_processor
    ):
    ...
    def test_get_movement_direction_from_odas_left(
        self, follow_task, mock_odas_processor
    ):
    ...
    def test_get_movement_direction_from_odas_backward(
        self, follow_task, mock_odas_processor
    ):
    ...
    def test_execute_task_success(
        ...
        with (
            patch("hexapod.task_interface.tasks.follow_task.logger") as mock_logger,
            patch("hexapod.task_interface.tasks.follow_task.time.sleep") as mock_sleep,
            patch(
                "hexapod.task_interface.tasks.follow_task.threading.Thread"
            ...
            mock_hexapod.wait_until_motion_complete.assert_called_once_with(
                stop_event=follow_task.stop_event
            )
    ...
    def test_execute_task_exception_handling(
        ...
        with (
            ...
            follow_task.stop_event = Mock()
            follow_task.stop_event.is_set.side_effect = [
                False,
                True,
            ...
            mock_odas_processor.get_tracked_sources_azimuths.side_effect = Exception(
                "ODAS error"
            )
            ...
            mock_logger.error.assert_called_once_with(
                "Error in follow loop: ODAS error"
            )
            mock_odas_processor.close.assert_called_once()
            mock_logger.info.assert_any_call("FollowTask completed")
