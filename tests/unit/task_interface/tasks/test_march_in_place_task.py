...
class TestMarchInPlaceTask:
    ...
    def test_init_custom_parameters(self, mock_hexapod, mock_lights_handler):
        ...
        callback = Mock()
        task = MarchInPlaceTask(
            mock_hexapod, mock_lights_handler, duration=10.0, callback=callback
        )
    ...
    def test_perform_march_with_duration(self, march_task_with_duration, mock_hexapod):
        ...
        with patch(
            "hexapod.task_interface.tasks.march_in_place_task.logger"
        ) as mock_logger:
            ...
            mock_hexapod.wait_until_motion_complete.assert_called_once_with(
                march_task_with_duration.stop_event
            )
            ...
            mock_logger.info.assert_any_call(
                "Started marching in place for 5.0 seconds in background thread"
            )
            mock_logger.info.assert_any_call(
                "Completed marching in place for 5.0 seconds"
            )
    ...
    def test_perform_march_stop_event_during_position_change(
        self, march_task_default, mock_hexapod
    ):
        ...
        with patch(
            ...
            mock_logger.warning.assert_called_once_with(
                "March task interrupted during position change."
            )
    ...
    def test_execute_task_success(
        self, march_task_with_duration, mock_hexapod, mock_lights_handler
    ):
        """Test successful execution of march task."""
        with (
            patch.object(march_task_with_duration, "_perform_march"),
            patch(
                "hexapod.task_interface.tasks.march_in_place_task.logger"
            ) as mock_logger,
        ):
    ...
    def test_execute_task_exception_handling(
        ...
        with (
            patch.object(
                march_task_with_duration,
                "_perform_march",
                side_effect=Exception("March failed"),
            ),
            patch(
                "hexapod.task_interface.tasks.march_in_place_task.logger"
            ) as mock_logger,
    ...
    def test_gait_parameters_customization(
        self, march_task_with_duration, mock_hexapod
    ):
    ...
    def test_different_durations(self, mock_hexapod, mock_lights_handler):
        ...
        for duration in durations:
            task = MarchInPlaceTask(
                mock_hexapod, mock_lights_handler, duration=duration
            )
            ...
            with patch("hexapod.task_interface.tasks.march_in_place_task.logger"):
                ...
                mock_hexapod.gait_generator.run_for_duration.assert_called_with(
                    duration
                )
    ...
    def test_march_task_threading_behavior(
        self, march_task_with_duration, mock_hexapod
    ):
    ...
    def test_march_task_stop_event_handling(
        self, march_task_with_duration, mock_hexapod
    ):
