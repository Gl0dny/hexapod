...
class TestSitUpTask:
    ...
    def test_perform_sit_up_stop_event_during_position_change(
        self, sit_up_task, mock_hexapod
    ):
    ...
    def test_perform_sit_up_stop_event_during_repetition(
        self, sit_up_task, mock_hexapod
    ):
    ...
    def test_sit_up_movement_calculations(self, sit_up_task, mock_hexapod):
        ...
        with (
            patch("hexapod.task_interface.tasks.sit_up_task.logger"),
            patch("hexapod.task_interface.tasks.sit_up_task.time.sleep"),
        ):
    ...
    def test_execute_task_success(self, sit_up_task, mock_hexapod, mock_lights_handler):
        ...
        with (
            patch.object(sit_up_task, "_perform_sit_up"),
            patch("hexapod.task_interface.tasks.sit_up_task.logger") as mock_logger,
        ):
    ...
    def test_execute_task_exception_handling(
        self, sit_up_task, mock_hexapod, mock_lights_handler
    ):
        """Test exception handling during sit up task execution."""
        # Make _perform_sit_up raise an exception
        with (
            patch.object(
                sit_up_task, "_perform_sit_up", side_effect=Exception("Sit up failed")
            ),
            patch("hexapod.task_interface.tasks.sit_up_task.logger") as mock_logger,
        ):
    ...
    def test_sit_up_stop_event_during_hold(self, sit_up_task, mock_hexapod):
        ...
        with (
            patch("hexapod.task_interface.tasks.sit_up_task.logger"),
            patch(
                "hexapod.task_interface.tasks.sit_up_task.time.sleep",
