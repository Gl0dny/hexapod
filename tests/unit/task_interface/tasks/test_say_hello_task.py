...
class TestSayHelloTask:
    ...
    def test_execute_task_success(self, say_hello_task, mock_hexapod, mock_lights_handler):
        """Test successful task execution."""
        with patch('hexapod.task_interface.tasks.say_hello_task.logger') as mock_logger, \
            ...
            mock_hexapod.wait_until_motion_complete.assert_called_once_with(say_hello_task.stop_event)
            mock_perform.assert_called_once()
            mock_logger.info.assert_any_call("SayHelloTask completed")

    def test_execute_task_stop_event_during_initial_movement(self, say_hello_task, mock_hexapod, mock_lights_handler):
    ...
    def test_execute_task_stop_event_during_infinity_wave(self, say_hello_task, mock_hexapod, mock_lights_handler):
    ...
    def test_execute_task_final_position(self, say_hello_task, mock_hexapod, mock_lights_handler):
        """Test that task moves to final position."""
        with patch('hexapod.task_interface.tasks.say_hello_task.logger'), \
            ...
            mock_hexapod.wait_until_motion_complete.assert_called_with(say_hello_task.stop_event)

    def test_execute_task_exception_handling(self, say_hello_task, mock_hexapod, mock_lights_handler):
        """Test task execution with exception handling."""
        with patch('hexapod.task_interface.tasks.say_hello_task.logger') as mock_logger, \
             patch.object(say_hello_task, '_perform_infinity_wave', side_effect=Exception("Test error")):
            
            say_hello_task.execute_task()
            
            # Should log the exception
            mock_logger.exception.assert_called_with("Say hello task failed: Test error")

    def test_say_hello_task_minimal_implementation(self, mock_hexapod, mock_lights_handler):
    ...
    def test_perform_infinity_wave_success(self, say_hello_task, mock_hexapod):
        ...
        with patch('hexapod.task_interface.tasks.say_hello_task.logger') as mock_logger, \
             patch('hexapod.task_interface.tasks.say_hello_task.time.sleep') as mock_sleep, \
            ...
            # Verify logging
            mock_logger.info.assert_any_call("Starting infinity wave motion using angle-based movement")
            mock_logger.info.assert_any_call("Original leg 0 angles: coxa=10.0°, femur=20.0°, tibia=30.0°")
            mock_logger.info.assert_any_call("Returning leg 0 to original angles")
            
            # Verify leg movements
            assert mock_hexapod.move_leg_angles.call_count >= 3  # At least preparation, animation, and return
            mock_hexapod.wait_until_motion_complete.assert_called()

    def test_perform_infinity_wave_stop_event_during_preparation(self, say_hello_task, mock_hexapod):
        ...
        with patch('hexapod.task_interface.tasks.say_hello_task.logger') as mock_logger, \
            ...
            mock_logger.info.assert_any_call("Infinity wave task interrupted during preparation.")

    def test_perform_infinity_wave_stop_event_during_animation(self, say_hello_task, mock_hexapod):
        ...
        with patch('hexapod.task_interface.tasks.say_hello_task.logger') as mock_logger, \
            ...
            mock_logger.info.assert_any_call("Infinity wave task interrupted during animation.")

    def test_perform_infinity_wave_stop_event_during_delay(self, say_hello_task, mock_hexapod):
        ...
        with patch('hexapod.task_interface.tasks.say_hello_task.logger') as mock_logger, \
            ...
            mock_logger.info.assert_any_call("Infinity wave task interrupted during delay.")

    def test_perform_infinity_wave_angle_calculations(self, say_hello_task, mock_hexapod):
        ...
        with patch('hexapod.task_interface.tasks.say_hello_task.logger') as mock_logger, \
            ...
            mock_hexapod.current_leg_angles = {0: (original_coxa, original_femur, original_tibia)}
    ...
    def test_perform_infinity_wave_math_calculations(self, say_hello_task, mock_hexapod):
        ...
        with patch('hexapod.task_interface.tasks.say_hello_task.logger') as mock_logger, \
            ...
            for i, call in enumerate(animation_calls[:5]):  # Check first 5 animation calls
                ...
                actual_coxa, actual_femur, actual_tibia = call[0][1], call[0][2], call[0][3]
    ...
    def test_execute_task_lights_handler_called(self, say_hello_task, mock_hexapod, mock_lights_handler):
    ...
    def test_execute_task_final_cleanup(self, say_hello_task, mock_hexapod, mock_lights_handler):
