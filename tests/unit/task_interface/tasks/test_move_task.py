...
class TestMoveTask:
    ...
    def test_execute_task_moves_to_zero_position_on_completion(
    ...
    def test_execute_task_handles_exception(self, mock_hexapod, mock_lights_handler):
        ...
        with patch.object(
            task, "_perform_movement", side_effect=Exception("Movement failed")
        ):
    ...
    def test_perform_movement_with_cycles(self, mock_hexapod, mock_lights_handler):
        ...
        mock_hexapod.gait_generator.create_gait.assert_called_once_with(
            "tripod", **mock_hexapod.gait_params["translation"]
        )
    ...
    def test_perform_movement_stop_event_during_position_change(
        self, mock_hexapod, mock_lights_handler
    ):
    ...
    def test_perform_movement_different_directions(
        self, mock_hexapod, mock_lights_handler
    ):
    ...
    def test_perform_movement_stop_event_logging(
        self, mock_hexapod, mock_lights_handler
    ):
        ...
        with patch("hexapod.task_interface.tasks.move_task.logger") as mock_logger:
            ...
            mock_logger.warning.assert_called_with(
                "Move task interrupted during position change."
            )
        ...
        with patch("hexapod.task_interface.tasks.move_task.logger") as mock_logger:
            with patch.object(task, "_perform_movement"):
                ...
                mock_logger.info.assert_any_call(
                    "Performing movement in forward direction."
                )
    ...
    def test_execute_task_exception_logging(self, mock_hexapod, mock_lights_handler):
        ...
        with patch("hexapod.task_interface.tasks.move_task.logger") as mock_logger:
            with patch.object(
                task, "_perform_movement", side_effect=Exception("Test error")
            ):
                task.execute_task()

                # Should log exception
                mock_logger.exception.assert_called_with(
                    "Error in MoveTask: Test error"
                )
