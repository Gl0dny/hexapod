...
class TestTaskInterface:
    ...
    def task_interface(self, mock_hexapod, mock_lights_handler, mock_button_handler):
        ...
        with patch('hexapod.task_interface.task_interface.Hexapod', return_value=mock_hexapod), \
             patch('hexapod.task_interface.task_interface.LightsInteractionHandler', return_value=mock_lights_handler), \
             patch('hexapod.task_interface.task_interface.ButtonHandler', return_value=mock_button_handler), \
    ...
    def test_init_custom_parameters(self, mock_hexapod, mock_lights_handler, mock_button_handler):
    ...
    def test_system_status(self, task_interface):
        ...
        task_interface.status_reporter.get_complete_status.assert_called_once_with(task_interface.hexapod)
        task_interface.lights_handler.listen_wakeword.assert_called_once()
    ...
    def test_wake_up(self, task_interface):
        ...
        task_interface.hexapod.move_to_position.assert_called_once_with(PredefinedPosition.ZERO)
    ...
    def test_calibrate(self, task_interface):
        ...
        with patch('hexapod.task_interface.task_interface.tasks.CompositeCalibrationTask') as mock_task_class:
            mock_task = MagicMock()
    ...
    def test_change_color(self, task_interface):
        ...
        task_interface.lights_handler.set_single_color.assert_called_once_with(ColorRGB.RED)
    ...
    def test_march_in_place(self, task_interface):
        ...
        with patch('hexapod.task_interface.task_interface.tasks.MarchInPlaceTask') as mock_task_class:
            mock_task = MagicMock()
            mock_task_class.return_value = mock_task
            task_interface.task = mock_task
    ...
    def test_move(self, task_interface):
        """Test move command."""
        with patch('hexapod.task_interface.task_interface.tasks.MoveTask') as mock_task_class:
            mock_task = MagicMock()
            mock_task_class.return_value = mock_task
            task_interface.task = mock_task
    ...
    def test_rotate(self, task_interface):
        """Test rotate command."""
        with patch('hexapod.task_interface.task_interface.tasks.RotateTask') as mock_task_class:
            mock_task = MagicMock()
            mock_task_class.return_value = mock_task
            task_interface.task = mock_task
    ...
    def test_follow(self, task_interface):
        """Test follow command."""
        with patch('hexapod.task_interface.task_interface.tasks.FollowTask') as mock_task_class, \
             patch('hexapod.odas.odas_doa_ssl_processor.ODASDoASSLProcessor') as mock_odas_class:
            mock_task = MagicMock()
            mock_task_class.return_value = mock_task
            task_interface.task = mock_task
    ...
    def test_sound_source_localization(self, task_interface):
        ...
        with patch('hexapod.task_interface.task_interface.tasks.SoundSourceLocalizationTask') as mock_task_class, \
             patch('hexapod.odas.odas_doa_ssl_processor.ODASDoASSLProcessor') as mock_odas_class:
            mock_task = MagicMock()
            mock_task_class.return_value = mock_task
            task_interface.task = mock_task
    ...
    def test_stream_odas_audio(self, task_interface):
        ...
        with patch('hexapod.task_interface.task_interface.tasks.StreamODASAudioTask') as mock_task_class, \
             patch('hexapod.odas.odas_doa_ssl_processor.ODASDoASSLProcessor') as mock_odas_class:
            mock_task = MagicMock()
            mock_task_class.return_value = mock_task
            task_interface.task = mock_task
    ...
    def test_sit_up(self, task_interface):
        """Test sit up command."""
        with patch('hexapod.task_interface.task_interface.tasks.SitUpTask') as mock_task_class:
            mock_task = MagicMock()
            mock_task_class.return_value = mock_task
            task_interface.task = mock_task
    ...
    def test_helix(self, task_interface):
        """Test helix command."""
        with patch('hexapod.task_interface.task_interface.tasks.HelixTask') as mock_task_class:
            mock_task = MagicMock()
            mock_task_class.return_value = mock_task
            task_interface.task = mock_task
    ...
    def test_stop_task_special_tasks(self, task_interface):
        ...
        for task_name in special_task_names:
            ...
            with patch.object(task_interface, 'request_unpause_voice_control') as mock_unpause, \
                 patch.object(task_interface, 'request_unblock_voice_control_pausing') as mock_unblock:
    ...
    def test_shut_down(self, task_interface):
        ...
        with patch('hexapod.task_interface.task_interface.NonBlockingConsoleInputHandler') as mock_input_class, \
             patch('hexapod.task_interface.task_interface.threading.Timer') as mock_timer_class, \
             patch('hexapod.task_interface.task_interface.threading.Thread') as mock_thread_class, \
    ...
    def test_shutdown_monitor_with_input(self, task_interface):
        ...
        mock_timer = MagicMock()
        mock_timer.is_alive.side_effect = [True, False]  # First call returns True, then False
        mock_input_handler = MagicMock()
    ...
    def test_perform_shutdown(self, task_interface):
        ...
        with patch('hexapod.task_interface.task_interface.os.system') as mock_system:
            task_interface._perform_shutdown(task_interface.hexapod, task_interface.lights_handler)
    ...
    def test_sleep_exception(self, task_interface):
        ...
        task_interface.hexapod.wait_until_motion_complete.side_effect = Exception("Wait error")
    ...
    def test_start_recording_exception(self, task_interface, mock_voice_control):
        ...
        task_interface.voice_control = mock_voice_control
        task_interface._recording_available = True
        mock_voice_control.get_recording_status.side_effect = Exception("Recording error")
    ...
    def test_stop_recording_exception(self, task_interface, mock_voice_control):
        ...
        task_interface.voice_control = mock_voice_control
        task_interface._recording_available = True
        mock_voice_control.stop_recording.side_effect = Exception("Stop recording error")
        
        # Should not raise exception, just log it
        task_interface.stop_recording()
    
    def test_start_recording_with_previous_recording(self, task_interface, mock_voice_control):
    ...
    def test_start_recording_with_previous_continuous_recording(self, task_interface, mock_voice_control):
    ...
    def test_stop_recording_no_active_recording(self, task_interface, mock_voice_control):
