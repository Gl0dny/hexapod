...
class TestStatusReporter:
    ...
    def test_get_complete_status_success(self, status_reporter, mock_hexapod):
        ...
        with patch.object(status_reporter, '_get_system_info') as mock_system, \
             patch.object(status_reporter, '_get_calibration_status') as mock_calibration, \
    ...
    def test_get_leg_positions_status_success(self, status_reporter, mock_hexapod):
        ...
        assert "Leg 0: Pos( 100.0,   50.0,  -80.0) mm | Angles( 10.0°,  20.0°,  30.0°)" in result
        assert "Leg 1: Pos( 120.0,   60.0,  -85.0) mm | Angles( 12.0°,  22.0°,  32.0°)" in result
        assert "Leg 2: Pos( 110.0,   55.0,  -82.0) mm | Angles( 11.0°,  21.0°,  31.0°)" in result
        assert "Leg 3: Pos( 105.0,   52.0,  -81.0) mm | Angles( 10.5°,  20.5°,  30.5°)" in result
        assert "Leg 4: Pos( 115.0,   58.0,  -83.0) mm | Angles( 11.5°,  21.5°,  31.5°)" in result
        assert "Leg 5: Pos( 125.0,   62.0,  -86.0) mm | Angles( 12.5°,  22.5°,  32.5°)" in result
    
    def test_get_leg_positions_status_error(self, status_reporter, mock_hexapod, caplog):
    ...
    def test_get_calibration_status_success(self, status_reporter, mock_hexapod):
        ...
        with patch('pathlib.Path.exists', return_value=True), \
            ...
            with patch('hexapod.task_interface.status_reporter.datetime') as mock_datetime:
                mock_datetime.fromtimestamp.return_value.strftime.return_value = "2022-01-01 00:00:00"
    ...
    def test_get_calibration_status_no_file(self, status_reporter, mock_hexapod):
        ...
        with patch('pathlib.Path.exists', return_value=False):
            ...
            assert result == "Calibration Status: Not calibrated (no calibration file found)"
    
    def test_get_calibration_status_no_is_calibrated_attr(self, status_reporter, mock_hexapod):
        ...
        with patch('pathlib.Path.exists', return_value=True), \
            ...
            with patch('hexapod.task_interface.status_reporter.datetime') as mock_datetime:
                ...
                assert "Status: Calibrated" in result  # Should default to calibrated if file exists
    ...
    def test_get_system_info_error(self, status_reporter, mock_hexapod, caplog):
        ...
        mock_hexapod.end_effector_radius = MagicMock(side_effect=Exception("System error"))
    ...
    def test_get_complete_status_with_errors(self, status_reporter, mock_hexapod):
        ...
        with patch.object(status_reporter, '_get_system_info', return_value="System Info: Error reading system data"), \
             patch.object(status_reporter, '_get_calibration_status', return_value="Calibration: Error reading calibration data"), \
             patch.object(status_reporter, '_get_imu_status', return_value="IMU: Error reading sensor data"), \
             patch.object(status_reporter, '_get_gait_status', return_value="Gait Status: No active gait"), \
             patch.object(status_reporter, '_get_movement_status', return_value="Movement: Error reading servo data"), \
             patch.object(status_reporter, '_get_leg_positions_status', return_value="Leg Positions: Error reading position data"):
