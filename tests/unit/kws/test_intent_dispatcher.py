...
class TestIntentDispatcher:
    ...
    def test_dispatch_invalid_intent(self, dispatcher):
        ...
        with pytest.raises(
            NotImplementedError, match="No handler for intent: invalid_intent"
        ):
    ...
    def test_handle_set_brightness_success(
        self, mock_parse, dispatcher, mock_task_interface
    ):
    ...
    @patch("hexapod.kws.intent_dispatcher.parse_percentage")
    def test_handle_set_speed_success(
        self, mock_parse, dispatcher, mock_task_interface
    ):
    ...
    @patch("hexapod.kws.intent_dispatcher.parse_percentage")
    def test_handle_set_accel_success(
        self, mock_parse, dispatcher, mock_task_interface
    ):
    ...
    def test_handle_march_in_place_invalid_time(
        self, dispatcher, mock_task_interface, caplog
    ):
    ...
    @patch("hexapod.gait_generator.base_gait.BaseGait")
    def test_handle_move_with_cycles(
        self, mock_base_gait, dispatcher, mock_task_interface
    ):
    ...
    @patch("hexapod.gait_generator.base_gait.BaseGait")
    def test_handle_move_with_duration(
        self, mock_base_gait, dispatcher, mock_task_interface
    ):
        ...
        dispatcher.handle_move(slots)
        mock_task_interface.move.assert_called_once_with(
            direction="forward", duration=30.0
        )
    ...
    def test_handle_rotate_with_angle(self, dispatcher, mock_task_interface):
        ...
        mock_task_interface.rotate.assert_called_once_with(
            turn_direction="left", angle=90.0
        )
    ...
    def test_handle_rotate_with_word_angle(self, dispatcher, mock_task_interface):
        ...
        dispatcher.handle_rotate(slots)
        mock_task_interface.rotate.assert_called_once_with(
            turn_direction="right", angle=90.0
        )
    ...
    def test_handle_rotate_with_duration(self, dispatcher, mock_task_interface):
        ...
        dispatcher.handle_rotate(slots)
        mock_task_interface.rotate.assert_called_once_with(
            turn_direction="right", duration=10.0
        )
    ...
    def test_handle_stream_odas_audio_default(self, dispatcher, mock_task_interface):
        ...
        mock_task_interface.stream_odas_audio.assert_called_once_with(
            stream_type="separated"
        )

    def test_handle_stream_odas_audio_custom_type(
        self, dispatcher, mock_task_interface
    ):
    ...
    def test_handle_stream_odas_audio_post_filtered(
        self, dispatcher, mock_task_interface
    ):
    ...
    def test_handle_start_recording_with_duration(
        self, dispatcher, mock_task_interface
    ):
    ...
    def test_exception_handling_in_handlers(self, mock_base_gait, dispatcher, caplog):
        ...
        dispatcher.task_interface.stream_odas_audio.side_effect = Exception(
            "Test error"
        )
        slots = {}
        dispatcher.handle_stream_odas_audio(slots)
