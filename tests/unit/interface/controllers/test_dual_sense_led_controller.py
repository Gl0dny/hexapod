...
class TestDualSenseLEDController:
    ...
    def test_init_success(self, mock_dualsense_class):
        ...
        mock_controller.enumerate_devices.return_value = [{"name": "DualSense Controller"}]
        mock_dualsense_class.return_value = mock_controller
        mock_dualsense_class.enumerate_devices.return_value = [{"name": "DualSense Controller"}]
    ...
    def test_connect_controller_exception(self, mock_dualsense_class, caplog):
        ...
        mock_dualsense_class.enumerate_devices.side_effect = Exception("Connection failed")
        
        controller = DualSenseLEDController()
        
        assert controller._connect_controller() is False
        assert controller.is_connected is False
        assert "Failed to connect to DualSense controller: Connection failed" in caplog.text
        assert "Make sure the controller is connected via USB or Bluetooth" in caplog.text
    ...
    def test_set_color_internal_exception(self, mock_dualsense_class, caplog):
        ...
        mock_controller = Mock()
        mock_controller.enumerate_devices.return_value = [{"name": "DualSense Controller"}]
        mock_controller.lightbar.set_color.side_effect = Exception("Color setting failed")
        mock_dualsense_class.return_value = mock_controller
        mock_dualsense_class.enumerate_devices.return_value = [{"name": "DualSense Controller"}]
    ...
    def test_cleanup_internal_exception(self, mock_dualsense_class, caplog):
        ...
        assert "Error deactivating DualSense controller: Deactivation failed" in caplog.text
