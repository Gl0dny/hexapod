...
class TestHandleButtonInteractions:
    ...
    def test_handle_button_interactions_long_press(self):
        ...
        mock_task_interface.button_handler.check_button.return_value = (
            "long_press",
            True,
        )
        ...
        with patch("hexapod.main.logger") as mock_logger:
            ...
            mock_logger.user_info.assert_called_with(
                "Long press detected, starting sound source localization..."
            )
    ...
    def test_handle_button_interactions_toggle_start(self):
        ...
        with patch("hexapod.main.logger") as mock_logger, patch("time.sleep"):
            handle_button_interactions(mock_task_interface)

            mock_task_interface.hexapod.move_to_position.assert_called_once_with(
...
class TestShutdownCleanup:
    ...
    def test_shutdown_cleanup_with_all_components(self):
        ...
        with (
            ...
            shutdown_cleanup(
                mock_voice_control, mock_manual_controller, mock_task_interface
            )
            ...
            mock_logger.critical.assert_called_with(
                "Stopping all tasks and deactivating hexapod..."
            )
...
class TestCreateApplicationComponents:
    ...
    def test_create_application_components_basic(self):
        ...
        with (
            patch("hexapod.main.clean_logs") as mock_clean_logs,
            ...
            task_interface, voice_control = create_application_components(
                mock_config, args
            )
            ...
            mock_task_interface.set_voice_control.assert_called_once_with(
                mock_voice_control
            )
    ...
    def test_create_application_components_with_print_context(self):
        ...
        with (
            ...
            mock_logger.debug.assert_called_with(
                "Print context flag detected, printing context"
            )
...
class TestInitializeManualController:
    ...
    def test_initialize_manual_controller_success(self):
        ...
        with (
            patch("hexapod.main.GamepadHexapodController") as mock_controller_class,
            patch("hexapod.main.logger") as mock_logger,
        ):
            ...
            result = initialize_manual_controller(
                mock_task_interface, mock_voice_control, mock_config, args
            )
            ...
            mock_logger.user_info.assert_called_with(
                "Manual controller started successfully"
            )
    ...
    def test_initialize_manual_controller_failure(self):
        ...
        with (
            ...
            mock_logger.warning.assert_called_with(
                "Failed to initialize manual controller: Controller error"
            )
            mock_logger.user_info.assert_called_with(
                "Falling back to voice control mode"
            )
...
class TestRunMainLoop:
    ...
    def test_run_main_loop_with_shutdown_event(self):
        ...
        with (
            patch("hexapod.main.shutdown_cleanup") as mock_cleanup,
            patch("hexapod.main.logger") as mock_logger,
        ):
            ...
            mock_logger.critical.assert_called_with(
                "Shutdown event detected, initiating shutdown"
            )
    ...
    def test_run_main_loop_with_manual_controller_voice_mode(self):
        ...
        with (
            patch("hexapod.main.handle_button_interactions") as mock_handle_buttons,
    ...
    def test_run_main_loop_without_manual_controller(self):
        ...
        with (
            ...
            sleep_calls = [
                call for call in mock_sleep.call_args_list if call[0][0] == 0.1
            ]
            assert (
                len(sleep_calls) >= 1
            ), f"Expected sleep(0.1) call, but got calls: {mock_sleep.call_args_list}"
...
class TestMainFunction:
    ...
    def test_main_success(self):
        ...
        with (
            patch("hexapod.main.create_main_parser") as mock_parser_func,
            patch("hexapod.main.Config") as mock_config_class,
            patch(
                "hexapod.main.create_application_components"
            ...
            mock_init_controller.assert_called_once_with(
                mock_task_interface, mock_voice_control, mock_config, mock_args
            )
    ...
    def test_main_config_validation_error(self):
        ...
        with (
            ...
            mock_print.assert_any_call(
                "For installation and setup instructions, see: INSTALL.md"
            )
