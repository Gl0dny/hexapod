...
class Calibration:
    ...
    def calibrate_servo_min(self, leg_index, joint_name, stop_event: Optional[threading.Event] = None):
        ...
        while not calibrated_min:
            if stop_event and stop_event.is_set():
                logger.error(f"Calibration interrupted during calibration of servo_min of Leg {leg_index} {joint_name}.")
                return
            try:
                ...
                for joint_name in ['coxa', 'femur', 'tibia']:
                    ...
                    if servo_min and servo_max:
                        if (992 * 4 <= servo_min <= 2000 * 4 and
                            ...
                            logger.error(f"Calibration values for leg {i} {joint_name} are invalid (servo_min: {servo_min}, servo_max: {servo_max}). Using default values.")
                            self.calibrate_servo(i, joint_name, 992 * 4, 2000 * 4)
                            logger.info(f"Set to default: servo_min=3968, servo_max=8000")
        except FileNotFoundError:
            logger.error(f"{self.calibration_data_path} not found. Using default calibration values. Run calibrate_all_servos() to set new values.")
        except json.JSONDecodeError as e:
            logger.error(f"Error decoding {self.calibration_data_path}: {e}. Using default calibration values. Run calibrate_all_servos() to set new values.")
